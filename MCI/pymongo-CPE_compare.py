...
def add_ID_to_dict_if_key_exist(a_dict, key, items):
    """Adds CVE ID to any dict if key exists."""
    if key not in a_dict:
        a_dict[key] = items
        last_CVE_ID = items
    elif key in a_dict:
        if last_CVE_ID != items:
            a_dict[key] = a_dict[key] + ', ' + items
                ...
                for children_index in range(len(nodes['children'])):
                    ...
                    try:
                        for cpe_match_index in range(len(children['cpe_match'])):
                            ...
                            add_ID_to_dict_if_key_exist(cpe_product_name, cpe_parsed_list[4], cve_ID)
                    except:
                        #pdb.set_trace()
...
with open('pymongo-CPE_name_check(only_underscore).csv', mode='w') as csv_file:
    fieldnames = ['MCI_Name', 'CVEs']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    writer.writeheader()
    for maven_names in intersection_set:
        writer.writerow({'MCI_Name': maven_names, 'CVEs': cpe_product_name[maven_names]})
